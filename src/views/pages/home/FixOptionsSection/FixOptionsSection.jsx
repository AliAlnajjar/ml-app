...
export default function FixOptionsSection(props) {
  ...
  useEffect(() => {
    if (appointmentFormRef.current) appointmentFormRef.current.scrollIntoView();
  }, [showAppointmentForm]);
  useEffect(() => {
    if (appointmentCardRef.current) appointmentCardRef.current.scrollIntoView();
  }, [appointmentFormDone]);

  useEffect(() => {
    if (sendDeviceFormRef.current) sendDeviceFormRef.current.scrollIntoView();
  }, [showSendDeviceForm]);

  useEffect(() => {
    if (sendDeviceCardRef.current) sendDeviceCardRef.current.scrollIntoView();
  }, [sendDeviceFormDone]);
    ...
    <Section title="Har du ikke mulighet til å komme til oss? ">
      ...
      <Grid container spacing={0} className={classes.card}>
        <Grid item xs={12} lg={7} xl={6} className={classes.textPart}>
          ...
          <Typography variant="body2" component="p" className={classes.desc}>
            Vi kan sende en tekniker til deg for skjermbytting av iPhone. Da
            slipper du å tenke på parkering rundt i byen, i tillegg til at du
            sparer tid for å gjøre andre ting mens teknikeren reparerer mobilen
            din. Vi operere foreløpig kun innenfor Bergen. Tjenesten gjelder kun
            for skjerm eller batteri på iphone når du forhåndsbestiller timen.
            Kunden betaler 100kr ekstra for kjøring. For reparasjon over 999
            skal kjøring være gratis.
          </Typography>
          <MyButton
            text="Bestill time"
            onClick={() => {
              setShowAppointmentForm(true);
              setAppointmentFormDone(false);
            }}
          />
        </Grid>
        <Grid item xs={12} lg={5} xl={6}>
          <CardMedia className={classes.mediaCard} image={WeComeToYouImage} />
        </Grid>

        {showAppointmentForm && !appointmentFormDone ? (
          <Grid item xs={12} className={classes.form}>
            <div ref={appointmentFormRef}>
              <MakeAppointmentForm onDone={handelAppointmentFormDone} />
            </div>
          </Grid>
        ) : appointmentFormDone ? (
      ...
      <Grid container spacing={0} className={classes.card}>
        <Grid item xs={12} md={6}>
          <CardMedia className={classes.mediaCard} image={PostenImage} />
        </Grid>
        ...
        <Grid item xs={12} md={6} className={classes.textPart}>
          ...
          <Typography variant="body2" component="p" className={classes.desc}>
            Hvis du har problemer med din telefon/tablet eller pc kan du sende
            den til oss for sjekk eller reparasjon, Når enheten skal returneres,
            pleier vi å bruke Bring. Det koster ca. 150 med sporing. Vi kan også
            sende den med brevpakka, men på kundens ansvar. Fyll ut vårt
            kontaktskjema.
          </Typography>
          <MyButton
            text="Fyll ut kontaktskjema"
            onClick={() => {
              setShowSendDeviceForm(true);
              setSendDeviceFormDone(false);
            }}
          />
        </Grid>
        {showSendDeviceForm && !sendDeviceFormDone ? (
          <Grid item xs={12} className={classes.form}>
            <div ref={sendDeviceFormRef}>
              <SendDeviceForm onDone={handelSendDeviceFormDone} />
            </div>
          </Grid>
        ) : sendDeviceFormDone ? (
